...
## 2026-06-29 — Coordinate Legibility Pack cut 1: provenance placards + §6 modals split · DOCTRINE §1.E v0.40 — STAGED

**Status: STAGED on `feature/clp-provenance` (off `main` @ `3da721d`, 3 commits); NOT merged — Codex Procedure 4 (surface + doctrine; §6 structural split) precedes merge per §10/L48, no push without operator confirmation per CLAUDE.md.** First cut of the Coordinate Legibility Pack (the operator's next product cycle). Recon-backed (`clp-recon` workflow) → operator-approved brief `~/Desktop/8ball/sessions/brief_clp_provenance_2026-06-29.md` with four locked decisions: **lead with provenance**, **richer per-coordinate notes** (method, not source-class buckets), **exclude placards from the §5.D PNG**, **reuse the existing labels toggle** (no new key).
