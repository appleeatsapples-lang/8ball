...
**Naming note: `كن فيكون / kun fayakun`.** Operator-coined Quranic phrase ("Be, and it is" — Allah's creative command) functions as the orchestrator codename. Treated as religious-scholarly Arabic vocabulary per MUHAB.md §8 #6 (do not gloss in tracked content); the Latin transliteration is a navigation aid for non-Arabic readers, not a translation. Appears in `orchestrator.md` H1 + `AGENTS.md` role table + the project's working vocabulary going forward. AGENTS.md "Orchestrator codename note" is the single canonical disambiguation paragraph; no other tracked file glosses the term.
...
- ✅ Local PII audit clean (47 files scanned, no hits).
...
**No code change, no test surface delta, no `core/` touch, no `index.html` touch, no `ui/*.js` touch.** This cycle is doctrine + scaffolding only. Test count stays 585; local PII audit clean; 47 files scanned by the local audit (no change from the previous post-v0.3.0.1 hygiene pass).
