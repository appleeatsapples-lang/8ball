...
## 2026-07-06 — DOCTRINE §4.A RETIRED: 18+ acknowledgment gate removed, v0.47→v0.48 — STAGED

**Status: STAGED on `age-gate-retirement` (off `origin/main` @ `87e3069`). Touches `DOCTRINE.md` — requires a cross-model audit before merge per CLAUDE.md's don't-do list. Not pushed.**
...
**Not yet done:** cross-model (Codex) audit — required before merge, this touches `DOCTRINE.md` and isn't a mechanical edit. Not pushed to origin. Branch is a fresh reparent off `origin/main` @ `87e3069` (not the session's original working branch, which had drifted behind origin after an earlier same-session PR merged) — cherry-picked cleanly, verified byte-identical diff scope before proceeding.

## 2026-07-06 — fix(profile): getMaturity reduces life path + name number before combining — STAGED

**Status: STAGED on `age-gate-retirement` (same branch as the §4.A entry above, off `origin/main` @ `87e3069`). Not pushed. No DOCTRINE/content/fixtures touch — no cross-model gate triggered by this commit alone.**
