...
`SUN_MEANINGS` and the private animal reuses `ANIMAL_MEANINGS` verbatim —
3 of the 33 diagnosis sheets still rendered an identical body twice), and
the 2.4MB `assets/cities.json` first-search latency.

**#5 — `content/meanings.v5.js` (ACTIVE), placement lines.** New release
= new file per §4: v5 re-exports every v4 export UNEDITED and adds one
table, `PLACEMENT_LINES` — one authored line per (placement, value) for
the two table-sharing surfaces, 12 rising lines reading the sign as the
front of the encounter (the ascendant's office) and 12 private-animal
lines reading the branch as the off-stage working register. Same append
contract as the v4 slot lines: shared base body byte-identical, the
placement's line appended after it in `entryFor`. Sun and public-animal
panels are pinned byte-identical to the registry — only the reusing
placements grow. DOCTRINE carries a mechanical meanings-v5 routing note
(§1.G) + content-version footer bump; the §1.G body/context boundary
question stays open and covers these lines identically.

**#8 — city dataset prefetch on field focus.** Investigation first:
`cities.json` is already column-packed (`{tz:[341], cities:[53308]}`),
so data-slimming had nothing honest to take. The cost was WHEN the
import starts — on the first debounced keystroke, putting a multi-second
fetch in front of the first suggestion list on slow links. `core/cities.js`
gains `warmCities()` (fire-and-forget wrapper over `loadCities()` —
shares its cache, in-flight promise and bounded importer sequence,
swallows its own rejection since the real search path owns the status
line); `ui/citysearch.js` fires it once on first birthplace-field focus
...
**Update, same session — PR #210 opened, two-lane review complete.**
sonnet MERGE WITH FIXES / opus MERGE WITH FIXES, reconciled MERGE WITH
FIXES. Sonnet found a real BLOCKER, independently reproduced by opus:
the first body render wrote two sheet nodes without extending
`ui/sheet.js`'s `valueNodes()`, so `clear()` left a stale citation in
live hidden DOM after an invalidated dyad resubmission — the PR #187 F1
class. Fixed (`005a97c`), regression-pinned, mutation-verified. Opus
exhaustively swept 1900–2100 (nine combinations exist; 3/3 at digit-sum
8 is the unique equal case; no year double-remaps; once-only held
everywhere) and found the injected-markup coverage hole (both body divs
deletable with the suite green) — closed with source pins on the markup
and production selectors, plus the guarded `primaryBody` write for
symmetry. Suite 57 files / **1954** tests green (this update supersedes
the 1952 above: +1 clear-inventory pin, +1 markup/selector pin). Record:
`audits/claude_relay_pr210_premerge_audit_2026-08-30_response.md`.
DOCTRINE §1.D v0.64's "open product call" clause is now answered by this
ship — the mechanical clause update is queued for the next doctrine PR;
...
## 2026-07-27 — Coverage-gap close: core data-table value pins + calendar suite + l48 predicate pin — SHIPPED

**Status: SHIPPED — #132 squash-merged to `main` as `404c296` and #133 as `b3b40c4`, both 2026-07-29; the STAGED language below is now fully superseded, corrected on sighting per the v0.48/v0.50/v0.51 footer precedent. The #132 half of this flip shipped separately as #138 (`f91959e`). **Both PRs cleared their gates on explicit controller overrides, not cross-model verdicts** — `audits/L48_override_pr132_2026-07-29.md` (sighting #10) and `audits/L48_override_pr133_2026-07-29.md` (sighting #11). No second model read either change. Sighting #11 states the recursion plainly: #133 modified the L48 mechanism itself under solo authority and was cleared by an instrument of that same mechanism, and its sharpest uncovered risk is the silent one — a guard wrong in the loose direction closes nothing and looks identical to a working gate, a path no real recycled-artifact PR has ever exercised. Both overrides enumerate what they leave uncovered.** Was: STAGED on `claude/test-coverage-analysis-27tc7k` (#132) and, stacked on it, `claude/l48-added-only-guard` (#133); merge is its own word. #132: `test` GREEN, `l48-gate` RED for the artifact reason only. #133 draws NO CI at all while stacked — `ci.yml` triggers on `pull_request: branches: [main]`, which filters on the base branch — so it must be retargeted to `main` after #132 lands, before it is merged. Both still need their own L48 artifact; the implementer lane cannot supply either. **Updated in-branch:** #132's artifact is now `audits/L48_override_pr132_2026-07-29.md` — an explicit controller override (L48 sighting #10), not a cross-model verdict; no second model has read this change. #133 still has none.

**Trigger.** The 2026-07-24 audit session (`audits/test_quality_audit_pr126_2026-07-24.md`, `audits/mutation_survivors_core_2026-07-24.md`) measured `core/` by mutation testing and found three modules whose data was executed but never value-asserted: `cities.js` 14.8%, `calendar.js` 42.7%, `countries.js` 61.8%. 222 country-name strings could be blanked, 171 arithmetic mutants in the ephemeris survived, and every centroid sign could flip while staying inside a ±90/±180 range check. Line coverage said 97–100% for all three — the gap only shows under mutation.

**The pins, and their oracles.** Independent of this repo, not snapshots of it. ICU region names (`Intl.DisplayNames`) cross-check both country-name tables: 208/224 entries in `core/cities.js` and 201/240 in `core/countries.js` match exactly, and the divergences are deliberate house renderings pinned by exact value with a staleness check, so an entry that stops diverging must leave the list. ICU's own Chinese calendar cross-checks `lunarNewYearDate` across all 201 years in range. Canonical jieqi windows (fixed month, nominal ±2 days, strict ordering, 29–32 day spacing) pin the solar terms. `assets/cities.json` cross-checks the legacy centroid signs: each centroid must sit closer to its own country's median city than to its own mirror.

**`tests/calendar.test.js` is new — `core/calendar.js` was the only `core/` module with no test file of its own,** its entire coverage being ten lunar-new-year and seven solar-term locks living in `tests/profile.test.js` (those stay where they are as the §3 HKO calibration checkpoints).

**`tests/l48_gate.test.js` is new, and closes the gap #131's own entry names** ("no tests — nothing in the suite executes workflow YAML"). It extracts the shipped grep predicate from `ci.yml` and pins what it must accept and reject: every historical verdict artifact on disk still passes (guarding against over-tightening), while the #126 renamed-record shape, a brief, loose dates, a missing model prefix, `pr12`/`pr126` prefix collisions, and paths outside `audits/` all fail. Also pins the job shape #131 established — own job, no `needs:`, full-history checkout, docs-only exemption, fail-closed exit. A CI gate loosens silently: nothing fails when it stops catching things, and both false-greens were caught only by a human noticing.

**Re-measured after the pins:** cities 14.8% → **96.3%**, countries 61.8% → **91.6%**, calendar 42.7% → **52.4%**; combined over the three 51.7% → **84.0%**, undetected 998 → 330 of 2066 valid mutants. The 330 that remain were characterised rather than waved off: all 114 in `countries.js` sit at the oracle's boundary (109 in countries with fewer than three cities in the dataset, 5 equatorial where the latitude check is deliberately exempt), and **zero** are reachable-but-uncovered. `calendar.js` moved least and is left at 52.4% deliberately — the ICU cross-check and jieqi windows kill gross drift but cannot kill a perturbation of a few hours inside a ±2-day window, and closing that needs exact per-year pins which, absent the HKO tables, would be a golden snapshot of the implementation asserting itself.

**Unresolved finding, recorded not smoothed over.** The ICU cross-check surfaced **four one-day lunar-new-year disagreements: 1916, 1954, 2027, 2030**. None is covered by the §3 HKO checkpoints (1900/1924/1950/1985/1990/2000/2010/2020/2024/2025), so which implementation is right is genuinely open. Both values are pinned in `ICU_DIVERGENCES` in `tests/calendar.test.js`, which locks the other 197 years and fails loudly if any of the four moves. Resolving it means checking those four years against the HKO tables — an operator/authority task, not an implementer one.

**#133, the added-only guard.** #131 tightened the artifact *shape*; the predicate still matches every file changed, so an artifact already on the base branch can be renamed or copied into a PR's name. #133 matches against `--diff-filter=A` with rename/copy detection instead. **Scope stated in the workflow itself:** this does NOT close #126's F1 on its own — there the rename was of a file created earlier on the same branch, so against the merge base both paths read as plain additions; the shape predicate is what rejects that. It closes the adjacent dodge, recycling a previous PR's verdict. Either way the gate stays a filename gate: it proves a correctly-named artifact was added, never that a review happened.

**Rebase history — this branch was restarted, not stacked.** #126 merged mid-session (2026-07-26) while four further commits sat on top of its already-merged history. Per the merged-PR rule the branch was restarted from current `main` and only genuinely-new work replayed. Dropped as already-landed-better: the F2 fix and F4 errata (#129 — `main`'s `rowSections` uses `Object.getOwnPropertyDescriptor`, defending against getter and prototype tricks the local version did not) and the gate tightening (#131). The centroid pins were rebased onto the 2026-07-25 strip: they now guard `tests/country_centroids.fixture.json` rather than fields `COUNTRIES` no longer carries — the fixture still feeds product code through the legacy rising replay, so a corrupted centroid there still yields a wrong rising sign. Pre-rebase history is at branch `backup/pre-rebase-44c6f6a`, safe to delete; three `push --delete` attempts were rejected by the session's proxy, so it is an operator-side chore.

**Corrections on record (L17 discipline).** Three claims made during the session were wrong and were corrected in place rather than quietly dropped: a combined pre-pin mutation score reported as 44.9% (count-derived value is 51.7%); a claim that #133 was "self-gating" (it draws no CI at all while stacked); and, in the earlier session records, two displayed percentages built by re-rounding the tool's already-2-decimal output — the defect #126's F4 recorded, whose errata #129 landed. The re-measurement section states its own figures are count-derived at full precision.

**Verification.** Full suite **1492/1492 across 43 files** on #132, **1496/1496** with #133's four added tests; each commit verified green in a clean checkout, not just in the working tree. The l48 workflow parses under `yaml.safe_load` and its step body under `bash -n`, and #133's four decision paths were simulated directly against the real grep line (added → pass; recycled → fail with the specific diagnostic; absent → fail; renamed record → fail on shape). The value pins were themselves falsified by hand-mutating the modules — the mutation re-run is the evidence. **Not verified:** the local PII audit did not run; it needs `audits/local_personal_data.txt`, which is gitignored and absent in a fresh container. The CI PII scan passes in-suite, but the operator-side pre-push audit is outstanding for both PRs.

**Scope (files):** `tests/calendar.test.js` (new), `tests/l48_gate.test.js` (new), `tests/cities_search.test.js`, `tests/countries.test.js`, `audits/mutation_survivors_core_2026-07-24.md` (re-measurement appended), `CLAUDE.md` (tests/ count 41 → 43, `repo_shape` lockstep), this file; plus `.github/workflows/ci.yml` on #133 only. **UNTOUCHED:** `core/`, `ui/`, `content/`, `index.html`, `DOCTRINE.md`, `tests/fixtures.json`. No product code changed on either branch; #133's only non-test change is the workflow predicate.

## 2026-07-26 — l48-gate tightening: response/override shapes only (false-green predicate closed) — SHIPPED

**Status: SHIPPED — squash-merged to `main` as `a4ce65c` (#131) on 2026-07-26; the STAGED language below is superseded by that merge, corrected on sighting per the v0.48/v0.50/v0.51/v0.55 footer precedent. **The acceptance test this entry set actually ran, both legs.** It staked the change on a live red-then-green sequence, and that is on the record rather than asserted: the RED leg is workflow run `30202684653` at `8be5665` (`test=success`, `l48-gate=failure`) — the tightened predicate correctly refusing the brief alone, which is exactly the #129 F2 false-green it exists to close — and the GREEN leg is run `30203501759`, `l48-gate=success` at 13:10:12Z once the verdict response landed under the required filename. The Codex verdict was **MERGE**, no defects found (`audits/codex_pr131_premerge_audit_2026-07-26_response.md`), and it recorded the RED leg while explicitly declining to claim the GREEN one, which had not yet happened at the time of the read. Both legs verified from check-run history for this flip rather than inherited from the entry's own claim. The residual the verdict named stands unchanged and is not closed by anything since: a self-authored file merely named `_response.md`, or an unauthorized override, still greens the gate, because content and provenance are unchecked — this is a filename gate.** Was: STAGED on `claude/l48-gate-response-only`; merge is its own word. The tightened gate applies to this very PR — expected RED after the brief commit, GREEN only once the verdict response lands; that live red-then-green sequence is the acceptance test. Trigger: the follow-up first staged at the #126 audit F1 (a 100%-similarity renamed session record satisfied the old predicate), corroborated by #129's F2 in both relay legs (the brief alone greened the gate).**
...
## 2026-07-26 — F2 fix cycle: `rowSections` fail-closed (§5.D guarantee enforced) + #126 audit errata — SHIPPED

**Status: SHIPPED — squash-merged to `main` as `437a101` (#129) on 2026-07-26; the STAGED language below is superseded by that merge, corrected on sighting per the v0.48/v0.50/v0.51/v0.55 footer precedent. **Both of the verdict's findings are closed, verified in the shipped source rather than taken from the disposition note.** The Codex verdict was MERGE WITH FIXES with two Mediums (`audits/codex_pr129_premerge_audit_2026-07-26_response.md`). **F1** — `rowSections` still trusted inherited and accessor properties, so a malformed cell could inherit `state: 'open'` and smuggle a paid value into both SVG and caption — is closed at `ui/share.js:191`, which now resolves each field through `Object.getOwnPropertyDescriptor` and accepts own **data** properties only; the adversarial pin the verdict asked for is `tests/share_surface.test.js:442`, labelled `#129 audit F1`, exercising an `Object.create`-inherited cell, a getter-backed cell, and a mixed own/inherited shape. **F2** — the brief and journal claimed L48 was RED until a response landed, while the live regex accepted the brief itself, permitting merge without an independent verdict — is closed by the gate tightening that shipped as #131 (`a4ce65c`), whose own entry above records the red-then-green proof. Worth keeping visible: F2 was a finding about this journal being wrong, and the correction it forced is the reason the gate now rejects a brief.** Was: STAGED on `claude/share-rowsections-fail-closed`; merge is its own word (L48 — code PR; the in-PR verdict is required by CLAUDE.md's gate letter and is filed on-branch, with the caveat that the live gate regex passed on the brief alone — the queued tightening debt, corroborated again by this cycle's verdict F2). Trigger: the open F2 recorded by the #128 queue-clear amendment; the controller's "F2 fix cycle" word executes the fix the #126 audit staged.**
...
## 2026-07-26 — DOCTRINE v0.55 footer flip: STAGED → SHIPPED naming `94cf1bc` (#127) — SHIPPED

**Status: SHIPPED — squash-merged to `main` as `b459fda` (#128) on 2026-07-26; the STAGED language below is superseded by that merge, corrected on sighting per the v0.48/v0.50/v0.51/v0.55 footer precedent. The edit it delivered is verified present: the DOCTRINE v0.55 changelog entry now carries its SHIPPED sentence naming `94cf1bc` (#127). **This flip went to Codex rather than claiming the mechanical-edit exemption, and the audit found something** — verdict MERGE WITH FIXES with one Medium (`audits/codex_pr128_premerge_audit_2026-07-26_response.md`): the new #126 release record cited its Codex verdict while omitting that #126 had merged *without* the required F2 fix, leaving the release log masking an L48 clearance failure. Closed in-branch by amending the entry, which now states plainly that F2 remained OPEN on `main` and names the follow-up; F2 itself was then fixed in #129.

**A precedent conflict this lane has to state against itself.** The DOCTRINE v0.56 and v0.57 footer corrections filed 2026-07-29 (#147, #146) claimed §10's mechanical-edit exemption — "Doctrine amendments go through the auditor before merge. Mechanical edits do not" — citing the v0.48 and v0.51 corrections. That citation is accurate but incomplete: **#128 is the nearest comparable case in time, it is the same class of change, and it did not take the exemption.** It was audited, and the audit returned a Medium. What that Medium was matters more than that it existed — a *truth defect in a release record*, which is precisely the surface a footer flip edits. So the premise the exemption rests on, that this class of change cannot be substantively wrong, is contradicted by the last time anyone checked. The precedent is genuinely split (v0.48 and v0.51 exempt, #128 audited), this lane picked the side that required nothing of it, and a cross-model read of #146 and #147 is worth commissioning on that basis.** Was: STAGED on `claude/doctrine-v055-footer-flip`; merge is its own word (L48). Trigger: the pending mechanical edit the #127 close-out entry below queued (v0.48–v0.51 footer precedent), run as its own doctrine-gated micro-cycle — path B of the 2026-07-26 post-#127 handoff.**
...
## 2026-07-20 — strict nine-number numerology + numerology-only harmony + doctrine v0.54 / calc v3 — RESUMED · SHIPPED

**Status: RESUMED and SHIPPED — squash-merged to `main` as `7d3285a` (#113); the STAGED/RESUMED language below is superseded by that merge, corrected on sighting per the v0.48/v0.50/v0.51 footer precedent.** Was: RESUMED and STAGED on `codex/all-coordinates-clickable` after the controller's later explicit nine-number product decision and explicit instruction to replay onto current main. The branch was fast-forwarded from the parked pre-#97 base to `6ff11b0` (#110/current `origin/main`) and its preserved worktree snapshot was replayed without rewriting the historical #110 PARKED record below. This entry prospectively supersedes that earlier parking decision; it does not erase it. The branch retains the v0.53 all-coordinate interaction work while replacing its cross-system numerology behavior. No commit, push, PR, merge, deploy, account, or third-party action was authorized or performed; §10/L48 independent cross-model audit remains required before merge.

**Calc v3.** `core/profile.js` now repeatedly digit-sums life path, expression/name number, soul urge, personality, birthday, and maturity to a terminal value in `1..9`; former master paths reduce `11 → 2`, `22 → 4`, and `33 → 6`. Maturity still combines the already-reduced life-path and name-number components. `core/engine.js` and `core/payments.js` accept only the nine active life paths, preserving exact anchors `1–3 low`, `4–6 mid`, `7–9 high`; 11/22/33 are rejected as invalid rather than treated as a fourth class. `tests/fixtures.json` changes in lockstep: master-path cases now pin 2/4/6, affected card-note brackets re-anchor, and the direct bracket table contains exactly nine values. Non-numerology calculations and the `(sun, public animal)` catalog index are unchanged. Stored profiles/readings need no schema migration because only reconstruction inputs persist and derived values are recalculated on open.

**Meaning and relation layer.** Immutable `content/meanings.v1.js` remains untouched as historical shipped content. Current `content/meanings.v2.js` exposes `NUMEROLOGY_MEANINGS` as exactly entries 1–9, removes active master-theme aliases, and makes all six numerology coordinates choose their two context partners only from the other five numerology axes. The panel labels that layer `WITH THE OTHER NUMBERS`; non-numerology coordinates retain `IN THIS SHEET`. New `content/concordance.v2.js` reuses all non-numerology v1 registries but retires the three master-reduction links from runtime: every active 1–9 life-path pair is honestly `unfiled` unless a named finite relation is added in a future version.

**Verification.** Pre-replay verification on the parked base was `npm test`: **1303/1303 (36 files)** plus a focused **305/305 (8 files)** — pre-rebase claims, superseded by the following actuals (F6). Post-replay, reconciled and rebased onto `1cf9260` (#112): full `npm test` **1364/1364 (37 files)** under vitest 4.1.9 — +17 over main's 1347 baseline, including the new calc-v3 facet-key migration pins. Live-browser pass on the rebased tree (2026-07-21, static serve of this worktree): a former-master DOB (raw digit sum 38 → old master 11) renders life path `2` on the free sheet; with a seeded stale pre-v3 facet position (`_v1` = `'2'`, t3 device) the first load clears the retired key and the note re-anchors `low` under `eight_ball_facet_index_v2`; the life-path panel opens `with the other numbers` with numerology-only partners; zero console errors. Pre-merge re-audit (L48/§10, in-PR, PR #113): fresh Claude lane via relay on audited head `535fe68` in a dedicated detached worktree — reviewer **SAFE TO MERGE**, reconciler **MERGE WITH ONE PRE-CONDITION** (a live-observed green run, since the relay sandbox gates node/npm; satisfied by the implementer runs recorded above and re-run post-absorb). Both findings (one low: assembled harmony/pillar output was voice-unscanned; one nit: missing `dayPillar` unresolved copy) absorbed at `faa93c2`, lifting the suite to **1366/1366 (37 files)**; verdict filed in-PR at `audits/claude_pr113_premerge_audit_2026-07-21_response.md`. Merge stays its own word. Direct edge coverage proves an empty name-number/vowel/consonant contribution and malformed 0/11/22/33 display inputs render unresolved rather than creating another number. The pre-replay live t3 card showed numerology values `3 / 6 / 4 / 2 / 1 / 9`—the former personality `11` rendered as `2`; its panel used `WITH THE OTHER NUMBERS`. Package-lock bytes remain unchanged.

## 2026-07-20 — value meaning in harmony with the full sheet + doctrine v0.53 — SHIPPED

**Status: SHIPPED — squash-merged to `main` as `7d3285a` (#113) as part of the calc-v3 PR, which carried this leg's work forward; the STAGED language below is superseded by that merge, corrected on sighting.** Was: STAGED on `codex/all-coordinates-clickable`; superseded for numerology by v0.54 while retained for non-numerology coordinates. The controller corrected the interaction objective directly: the panel must say what a value means in harmony with the others, not how it is derived. The same narrow local implementation override applies. No commit, push, PR, merge, deploy, account, or third-party action was authorized or performed; §10/L48 independent cross-model audit remains required before merge.

**Behavior and content.** Every resolved sheet coordinate now opens two layers: its registered value meaning, then a separately labeled `in this sheet` synthesis. The synthesis reads two available neighboring meanings through fixed coordinate roles—for example, Earth stability as the material tempo beside Capricorn ambition as the outward agenda and Snake discretion as the public instinct; name-number 6 care as the full-name pattern beside life-path 3 expression and soul-urge 4 structure. `content/meanings.v2.js` reuses the immutable v1 arcana/sign/animal/number tables, adds five element entries, grammatical number-theme aliases, and fourteen finite context-role/partner records. Day/hour pillars combine their registered animal and element meanings. No resolved panel contains derivation copy. Unresolved and sealed cells state that no value meaning is available without fabricating or leaking one; catalog remains excluded.

**UX and verification.** The panel separates the base meaning from the integrated read with an `IN THIS SHEET` label, preserving scan hierarchy in the existing inline expansion. All 14/14 tier-3 cells opened live; all twelve resolved cells showed contextual synthesis, the two unresolved cells showed their honest empty state, and zero panels contained `derived by`. Live visual inspection confirmed the longer panel fits the 360px card without horizontal overflow or clipped copy; keyboard semantics, 44px targets, focus treatment, Escape/close behavior, and focus restoration remain intact. Pre-replay `npm test`: **1301/1301 (36 files)**. New content-policy coverage scans the v2 element/context strings. No `index.html`, tier, payment, persistence, share, network, or dependency change belongs to this layer.

## 2026-07-20 — all coordinate compartments interactive + doctrine v0.52 — SHIPPED

**Status: SHIPPED — squash-merged to `main` as `7d3285a` (#113) as part of the calc-v3 PR, which carried this leg's work forward; the STAGED language below is superseded by that merge, corrected on sighting.** Was: STAGED on `codex/all-coordinates-clickable`. Direct controller request authorized a local implementation and the narrow Codex lane exception for this task. No commit, push, PR, merge, deploy, account, or third-party action was authorized or performed. Because this amends §1.G and changes user-visible behavior, the ordinary §10/L48 independent cross-model audit remains required before merge.

**Behavior.** All fourteen specimen-sheet coordinate cells now use the same pointer and keyboard interaction. Resolved cells open registered value meaning and deterministic context; unresolved and sealed states remain honest and value-safe. The catalog numeral remains excluded.

**Accessibility and visual verification.** Every cell has button semantics, `tabindex=0`, `aria-controls`, `aria-expanded`, a visible focus treatment, Enter/Space activation, Escape dismissal, focus restoration, and a 44px minimum target. Pre-replay live verification opened all 14/14 cells; Enter opened and Escape closed; the 360px card stayed inside the viewport with no horizontal overflow.
...
**[Superseded, 2026-07-26 — annotation added at #124 merge-prep, per the #127 Codex cross-PR note (`audits/codex_pr127_premerge_audit_2026-07-26_response.md`)]:** the decision recorded below was reversed by a controller correction carrying the same 2026-07-20 date — the DOCTRINE §1.B v0.54 nine-number amendment shipped the "permanently parked" leg as calc v3: masters retired as terminal values (`11 → 2`, `22 → 4`, `33 → 6`), and the active registries became `content/meanings.v2.js` (whose `NUMEROLOGY_MEANINGS` is the nine-entry `1..9` subset, not the twelve-entry shape the kept-path paragraph below predicts) and `content/concordance.v2.js` (master-reduction links retired). Calc v3 remains current on `main` (DOCTRINE §3, v0.54 through v0.55). The entry below stays verbatim as the historical record of the pre-correction decision; nothing in it describes current state.
