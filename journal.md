...
**Mechanics (`.github/workflows/ci.yml`, one new step after the #83 DOCTRINE/content gate).** On `pull_request` events whose head branch starts with `claude/`: docs-only PRs (every changed file `.md`) are exempt — they ship no behavior; every other `claude/*` PR must ADD an `audits/` file naming its own PR number. Two accepted shapes: the cross-model verdict response (`audits/<model>_prNN_premerge_audit_<date>_response.md` — filed in-repo now, not vault-only, per the #83/#84 in-repo-artifact precedent) or an explicit override (`audits/L48_override_prNN_<date>.md`) — and the override file IS the L48 sighting log, so overriding stays possible but self-documents by construction. CI checks presence, not content — same philosophy as the #83 gate; content discipline stays with the pen. Non-`claude/*` lanes (`codex/*`, manual) are untouched.

**Verification.** Suite 1232/1232 (33 files) vitest 4.1.9 unchanged (workflow + journal touch only); local PII audit clean (91 files at the pre-artifact push). The close-out PR itself demonstrates the gate live: it touches `ci.yml` (not docs-only), so its own merge requires its own in-PR audit artifact — filed once the pre-merge relay verdict lands. **Scope (files):** `.github/workflows/ci.yml`, `journal.md`, plus the PR's own `audits/` verdict artifact. **UNTOUCHED:** `core/`, `ui/`, `content/`, `tests/`, DOCTRINE.
