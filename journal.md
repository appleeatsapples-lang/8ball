...
## 2026-06-12 — v0.7.0 compartment card (constant-skeleton specimen sheet) · DOCTRINE v0.37 — STAGED

**Status: STAGED on branch `v0.7.0-compartment-card` — NOT merged.** Gate per brief §6: CC implement → full suite green → push → PR (no merge) → **Codex Procedure 4 MANDATORY** (doctrine + paid-surface; orchestrator stages the audit brief — expected hooks: DOM purity at every rung / §1.D-amendment-vs-implementation parity / free-surface invariant / seal-unres distinction / animation idempotence / L17 lineage) → CiC live-fire post-merge → operator merge only on AUDIT-CLEARED (L48). Brief: `~/Desktop/8ball/sessions/brief_v070_compartment_card_2026-06-12.md` (+ design `design_compartment_card_2026-06-12.md` + off-repo mock). **Step-0 prerequisite verified before branching:** PR #37 MERGED to main 2026-06-12T06:04:34Z; origin/main `.reads-chip` carries no `position: absolute` and `.flip-inner` is `display: grid`.
