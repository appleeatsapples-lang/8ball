...
## 2026-07-01 — test hardening: §4.A age-gate behavior + dead-branch coverage + PII positive-fires + throw-message pins (+17 tests) — STAGED

**Status: STAGED on `test-hardening` (off `origin/main` @ `d83ec99`) — NOT merged; 4 commits. Operator merges.** Tests-only — 7 test files, +287/−17, **1118 → 1135 tests**. No `core/` / `ui/` / `index.html` / `content/` / `fixtures.json` / DOCTRINE touch ⇒ cannot affect prod, no Codex. Picked under the operator's "test hardening" steer.
