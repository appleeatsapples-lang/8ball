...
## 2026-06-29 — Coordinate Legibility Pack cut 2: ATLAS legend (system-name placards) — STAGED

**Status: STAGED — on branch `claude/compassionate-dijkstra-3c0dd4`, branched off `main` @ `86ad6a0` (CLP cut 1 close-out). Committed, NOT pushed/merged — awaiting operator review → PR (no merge) → Codex Procedure 4 → merge → close-out.** Second cut of the Coordinate Legibility Pack, off the operator-approved brief `~/Desktop/8ball/sessions/brief_clp_provenance_2026-06-29.md` §3.2 / §6 (ATLAS = the scoped next cut after provenance). Lane: CC (operator picked ATLAS over density/reach/brief-first; then picked **Shape A** — a separate gated legend line — over folding the system name into the shipped provenance placard).
...
**Tests: 1087 → 1100 (+13), 23 files (+`tests/atlas.test.js`), all green** (`npm test` exit 0). Local PII audit clean (70 files). `wc -l index.html` = **1456 ≤ 1500** (cut 1 left 55 lines of headroom; the `.coord-atlas` CSS block spent ~11).
