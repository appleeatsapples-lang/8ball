...
## 2026-07-05 — test-helper de-fork: canonical voice-register module + repo-shape count pin — STAGED

**Status: STAGED on `claude/more-suggestions-n1juc1` (off `origin/main`). Tests + docs only — no `core/` / `ui/` / `index.html` / `content/` / `fixtures.json` / DOCTRINE touch ⇒ no Codex P4; `package.json` unbumped.** Operator asked (on a phone) for "more suggestions"; the returned list recommended the #74 deferred "tests/helpers consolidation" (A1) + a new repo-shape count pin (B5), and the operator said go. Both shipped here; the wider DOM-mock consolidation is deliberately left as a separate cut (see below).
...
**Deliberately NOT done — separate cut.** The DOM-mock half of A1 (`makeEl`/`makeClassList`/`makeElement`/`makeStorage`/`makeNode`/`makeRefs` duplicated with drifted signatures across modals/modal_a11y/tiers/meanings_behavior/citysearch) is genuinely wide and signature-divergent — reconciling it means merging behavior differences the tests depend on, not a mechanical move. Left for a focused session, consistent with the #74 close-out calling it a "wide, separate cut."

**Verification.** `npm test` → **1226 green (33 files)**; `git diff --check` clean; public PII/privacy/dependency scans green (they run inside the suite). Local `run_local_audit.sh` not runnable in the remote container (operator-local pattern file) per the standing note — no PII-shaped strings added anyway (test-helper + count-pin only). **Scope (files):** new `tests/helpers/voice-register.js`, new `tests/repo_shape.test.js`, `tests/profile.test.js`, `tests/meanings_content.test.js`, `tests/provenance.test.js`, `tests/atlas.test.js`, `CLAUDE.md`, this entry. **UNTOUCHED:** all of `core/` / `ui/` / `content/` / `index.html` / `fixtures.json` / DOCTRINE. **Next:** operator review/merge → flip STAGED → SHIPPED.
