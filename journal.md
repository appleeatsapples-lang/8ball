...
## 2026-06-25 — v0.38 free life-path split (override #3) · DOCTRINE v0.38 — IN-FLIGHT (PR open, awaiting Codex + operator merge)

**Status: branch `feature/free-lifepath-split`, PR open, NOT merged.** Gate: go-green DONE (1052/1052, 20 files, `npm test` exit 0) → this journal entry (§8 DOCTRINE-touch gate) → commit/push/PR → **Codex Procedure 4 MANDATORY** (doctrine + free-surface change; expected hooks: §1.D-v0.38-vs-implementation parity, free = DOB-only invariant preserved, catalog isolation `(sunSign, animal)` unchanged, §5.D share-PNG PII basis, L17 lineage, version-truth) → operator merge only on AUDIT-CLEARED (L48). Brief: `~/Desktop/8ball/sessions/override3_free_lifepath_split_2026-06-12.md`.
