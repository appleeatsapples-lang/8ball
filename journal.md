...
**Status: STAGED on `age-gate-retirement` (off `origin/main` @ `87e3069`). Cross-model audit complete (below); 4 must-fix findings corrected in a follow-up commit. Still not pushed to origin.**
...
**Not yet done:** still not pushed to origin — that and PR routing are the operator's call. Branch is a fresh reparent off `origin/main` @ `87e3069` (not the session's original working branch, which had drifted behind origin after an earlier same-session PR merged) — cherry-picked cleanly, verified byte-identical diff scope before proceeding.
