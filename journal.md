...
## 2026-07-03 — docs follow-up: 8BALL.md §3 row 9 content-version — pre-v0.36 locked/unlocked framing → compartment specimen-sheet ladder — STAGED

**Status: STAGED on `claude/optimistic-pascal-97d354` (off `origin/main` @ `ed7688a`, rebased over the F01–F07 cut #65 + its close-out #66) — [#67](https://github.com/appleeatsapples-lang/8ball/pull/67), awaiting controller merge.** This is the journal top-insert the #65 close-out's "follow-up in flight (separate lane)" line expects — reconciled here. Touches `8BALL.md` (one row) + this entry — docs only; no `core/` / `ui/` / `index.html` / `content/` / `tests/` / `fixtures.json` / DOCTRINE touch ⇒ no Codex P4; `package.json` unbumped. Closes the pre-existing drift #65's pre-push review logged as out of F01–F07 scope.
