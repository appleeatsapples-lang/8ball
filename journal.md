...
## 2026-07-27 — Coverage-gap close: core data-table value pins + calendar suite + l48 predicate pin — SHIPPED (#132) · STAGED (#133)

**Status: #132 SHIPPED — squash-merged to `main` as `404c296` on 2026-07-29; the STAGED language below is superseded for #132 by that merge, corrected on sighting per the v0.48/v0.50/v0.51 footer precedent. #133 remains STAGED and unmerged: it still draws no CI, and now that its base `claude/test-coverage-analysis-27tc7k` has merged it must be retargeted to `main` before it can be reviewed or merged, and still needs its own L48 artifact. #132 cleared its gate on an explicit controller override (`audits/L48_override_pr132_2026-07-29.md`, sighting #10) and NOT a cross-model verdict — no second model read this change, and the override enumerates what that leaves uncovered.** Was: STAGED on `claude/test-coverage-analysis-27tc7k` (#132) and, stacked on it, `claude/l48-added-only-guard` (#133); merge is its own word. #132: `test` GREEN, `l48-gate` RED for the artifact reason only. #133 draws NO CI at all while stacked — `ci.yml` triggers on `pull_request: branches: [main]`, which filters on the base branch — so it must be retargeted to `main` after #132 lands, before it is merged. Both still need their own L48 artifact; the implementer lane cannot supply either. **Updated in-branch:** #132's artifact is now `audits/L48_override_pr132_2026-07-29.md` — an explicit controller override (L48 sighting #10), not a cross-model verdict; no second model has read this change. #133 still has none.
