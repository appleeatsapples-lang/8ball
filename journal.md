...
## 2026-07-01 — local-audit hardening: pii_scan sentinel self-exclusion (scan-scope truth) — STAGED

**Status: STAGED on `fix/local-audit-pii-sentinels` (off `origin/main` @ `65403c3`, rebased over #59 + #60 onto `c1a80f6`) — PR [#62](https://github.com/appleeatsapples-lang/8ball/pull/62); merge stays controller-gated.** Touches `audits/run_local_audit.sh` (scan-scope exclusion + rationale comment) + `audits/LOCAL_PII_AUDIT.md` (v0.2) + this entry. Audit-tooling only — no product surface, no behavior change in `core/` / `ui/` / `index.html` / `tests/`; `package.json` unbumped. Closes the max-evolution handoff §5 flagged HIGH (local-audit fragility vs the #56 positive-fire sentinels). Sibling cut this session: the og-image specimen refresh (#59 — SHIPPED, close-out in its entry below).

**The defect (proven, not theoretical).** #56's test-hardening added positive-fire sentinels to `tests/pii_scan.test.js` — leak-SHAPED strings (name tokens, labeled-DOB-shaped samples) that pin each banned pattern still fires. The PUBLIC scan self-excludes that file (`SKIP_FILES`); `run_local_audit.sh` did not. Canary proof: appending a DOB-shaped pattern matching one sentinel to the (gitignored) pattern file → the pre-fix script fires on `tests/pii_scan.test.js:151` — a false positive on a string that exists precisely to test the guard, where any "fix" would corrupt either the sentinel or the pattern list. Post-fix script: clean on the same canary. Pattern file restored byte-exact afterward (cmp-verified against the main-checkout original).

**The fix.** `run_local_audit.sh` mirrors the public scan's self-exclusion: `tests/pii_scan.test.js` joins the scan-scope exclusion list. Compensating controls, stated in the doc: the scanner file is guarded by review on every diff plus its own BANNED-driven positive-fire discipline (a pattern with no firing sample fails loudly) — grepping the guard's own examples was never real coverage.

**Doc truth (`LOCAL_PII_AUDIT.md` v0.2):** new "Scan scope" section (full exclusion list + why); new pattern-class boundary — the local pattern file should carry only patterns with ZERO legitimate tracked occurrences (DOB / address / family / ID classes); operator-name shapes belong to the public scan's banned+allow system, since they appear legitimately in tracked content (the LICENSE copyright line, `MUHAB.md` governance references) and a raw name-grep false-fires on files the public scan deliberately allows (canary-verified during this cut). The example block's "[your full real name]" line is removed accordingly. The local audit keeps NO allow-list on purpose: a hit must always mean "remove it."

**Scan-scope note (disclosed per the CLAUDE.md journal-note rule):** this narrows the LOCAL audit's file set by exactly one file — the scanner itself, which structurally cannot pass a leak-shaped grep. The public scan's `BANNED` list, its allow-lists, and the privacy scan's `FORBIDDEN` / `LOCALSTORAGE_KEY_ALLOW` surfaces are untouched.

**Verification:** local audit clean — 73 files scanned (74 → 73, exactly the one excluded file); canary pre/post proof above; full suite green (count in the commit); no tracked-file change outside `audits/` + this entry.

**Parallel-cut reconciliation (disclosed).** The chip session running concurrently staged the SAME one-line script exclusion as [#61](https://github.com/appleeatsapples-lang/8ball/pull/61) (its own journal entry + doc bump; identical `grep -vE` line). This entry absorbs #61's two stronger arguments, credited: (a) **why exclusion, not sentinel-fragmenting** — fragmenting the sentinel strings cannot close the hole because the `BANNED` regex literals themselves carry the same tokens in plain source, and obfuscating the canonical pattern list defeats the file's documented purpose; (b) **what is given up** — the local grep no longer covers `pii_scan.test.js`, so a real personal-data paste into that ONE file would be caught by neither layer; accepted because leak-shaped content is the file's function, edits to it are already high-scrutiny (CLAUDE.md don't-do + review), and the alternative failure mode (weakening sentinels to appease the grep) is strictly worse than the one-file coverage gap. Exactly one of #61/#62 merges; the other closes as superseded — this one additionally carries the canary proof, the doc pattern-class boundary, and the example-block fix.

**UNTOUCHED:** `tests/` (incl. `pii_scan.test.js` itself), `core/`, `ui/`, `index.html`, `content/`, `DOCTRINE.md`, `fixtures.json`, `package.json`. **Next:** controller picks #61 or #62 → merge → close-out STAGED→SHIPPED (+ close the sibling PR as superseded).
