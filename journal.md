...
**Verification.** Suite green on fresh run: **1163 tests (25 files, +1)**; local PII audit **clean (73 files)**; `index.html` **1497** (≤1500); no fetch / analytics / new localStorage key; every new string holds the §2 clinical register (no destiny/fate/secret/reveal-your, no FOMO/urgency/price-anchoring). **Scope (files):** `index.html`, `tests/payments_markup.test.js`, `DOCTRINE.md`, `8BALL.md`, `README.md`, `audits/RELEASE_CHECKLIST.md`, this entry. **UNTOUCHED:** everything else. **Next:** controller merge → close-out flip STAGED → SHIPPED.
