...
**Status: STAGED on `fix/local-audit-pii-sentinels` (off `origin/main` @ `65403c3`); merge stays controller-gated.** Touches `audits/run_local_audit.sh` (scan-scope exclusion + rationale comment) + `audits/LOCAL_PII_AUDIT.md` (v0.2) + this entry. Audit-tooling only — no product surface, no behavior change in `core/` / `ui/` / `index.html` / `tests/`; `package.json` unbumped. Closes the max-evolution handoff §5 flagged HIGH (local-audit fragility vs the #56 positive-fire sentinels). Sibling cut this session: the og-image specimen refresh (PR #59, entry on its own branch).
...
**UNTOUCHED:** `tests/` (incl. `pii_scan.test.js` itself), `core/`, `ui/`, `index.html`, `content/`, `DOCTRINE.md`, `fixtures.json`, `package.json`. **Next:** controller merge → close-out STAGED→SHIPPED.
## 2026-07-01 — og-image specimen refresh: the social unfurl now carries the real free-tier sheet (+ og asset-parity pins) — STAGED

**Status: STAGED on `feature/og-image-specimen-refresh` (off `origin/main` @ `65403c3`) — NOT pushed; push + PR + merge stay controller-gated per CLAUDE.md.** Touches `assets/og-image.png` (bytes replaced in place, 25,648 → 57,139) + `tests/reach_surface.test.js` (+2 pins). No `index.html` / DOCTRINE / `content/` / `core/` / `fixtures.json` touch; `package.json` unbumped. Reach lane (REACH_CONTROL: features frozen, reach only) — closes the carried "Item 3 (OG asset refresh)" deferral from the reach-pass entry of 2026-06-29 and the max-evolution handoff §5 "stale og-image" item (flagged there as the highest-leverage reach miss). The deferral's stated blocker — "no headless rasterizer without a dependency" — was already obsolete: the vault's own `render_extended_specimen.mjs` pattern (headless Chrome, off-repo, zero deps) covers it.
