...
## 2026-06-29 — Cycle A: result-screen desktop side-rail + §6 split (`ui/labels.js`) — ADOPTED onto main — STAGED

**Status: STAGED on the CC worktree branch (`feature/cycle-a-side-rail`, branched off `main` @ `504ebf7` = post-reach-pass); NOT merged — Codex Procedure 4 (§6 structural split) precedes merge per §10/L48, no push without operator confirmation per CLAUDE.md.** Layout-only fix for a **pre-existing v0.7.0 condition** (NOT a regression): on the result screen the controls stack below the full-sheet specimen card (~565–576px intrinsic height), so SHARE fell below the fold at 1280×800 and 1280×720. Surfaced by CiC during the reach-pass-v1 review; its own cycle.
