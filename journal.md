...
- `audits/RELEASE_CHECKLIST.md` — two touches: §5.B Buy Link check (line :15) rewrites from LS-`checkout[success_url]` query-param mechanism to Gumroad bare-href + product Content-tab Button mechanism per DOCTRINE §5.B Call 2 v0.28; post-merge smoke-test wording (line :37) updates target domain from `lemonsqueezy.com` to `theeightball.gumroad.com` matching live deploy state.
...
- `agents/verifier.md` — three touches: §2 payment-flow section header (lines :98-:107) updates "Lemon Squeezy" → "Gumroad" / generic-processor framing; procedure block rewritten to reference Gumroad checkout flow + bare-href mechanism per DOCTRINE §5.B Call 2 v0.28; payment-processor domain allowlist (line :143) drops `app.lemonsqueezy.com` and `*.lemonsqueezy.com` entries, retains `theeightball.gumroad.com` as active processor + `connect.stripe.com` as Stripe-Connect-via-Gumroad surface. Audit history block preserved verbatim per L17.
...
- `git diff --stat main..HEAD` post-commit → 13 files changed, ~29 insertions, ~26 deletions (per pre-commit working-tree diffstat at chat-30 close).
