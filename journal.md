...
## 2026-07-01 — boot-guard correctness: stored-profile city leak fix (retroactive-audit MAJOR) + throw-message pins — STAGED

**Status: STAGED on `fix/boot-guard-selectedcity-leak` (off `origin/main` @ `525ab0b`) — NOT merged; a PROPER PR this time (the boot-guard it fixes reached `main` by a direct-push slip — process note below).** Touches `index.html` + `tests/profile.test.js` + `tests/dob_validation.test.js`. No DOCTRINE / `content/` / `core/` / `fixtures.json` touch; `package.json` unbumped. Operator merges.
...
**Tests 1118 green; PII clean (71); `index.html` 1491 (≤1500).** **Scope:** `index.html`, `tests/profile.test.js`, `tests/dob_validation.test.js`, this entry. **Next:** operator merge → close-out.
