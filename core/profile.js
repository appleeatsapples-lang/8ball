...
// calc v5 (DOCTRINE §3). The Pythagorean table is defined over the 26 Latin
// letters, so every name reducer must read the SAME canonical letter
// sequence — otherwise two spellings of one name reduce to different
// coordinates. "José" typed as NFC (é = U+00E9) and as NFD (e + U+0301) is
// the same name to Unicode, to the keyboard that produced it and to the
// person who owns it; which one arrives is decided by keyboard, IME or OS,
// never by the person.
//
...
// including maturity — to `null` (`—` on the sheet), the §1.B v0.62
// absent-letter-class guard applied to the whole alphabet rather than to
// one letter class. No coordinate is invented from code points the table
// cannot read.
...
export function buildProfile(name, dobIso, opts) {
  ...
  // calc v5: NFC is the same contract seen from the display side — the
  // retained name is stored in one canonical form, so two spellings of one
  // name produce one profile end to end, not merely one set of numbers.
