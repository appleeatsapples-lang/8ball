...
3. Privacy (§5) — localStorage allow-list, no unpermitted network calls (only §5-permitted same-origin lazy loads + the §5.B user-initiated feedback POST — the Gumroad Buy Link redirect this line once also named was retired along with the whole commerce surface by the 2026-09-02 free amendment, doctrine §1.D v0.71; one permitted call remains, not two), no third-party fonts/scripts
