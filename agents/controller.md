...
- 2026-07-19 — Registry + Concordance MVP one-time override recorded. The controller authorized Codex to consolidate the locally explored registry into the Saved Readings surface, amend doctrine/controller/privacy documentation, implement and verify in `~/dev/8ball`, and create a branch plus local commit. Authority excludes calculation-engine changes, push, PR, merge, deploy, production, accounts, cloud storage, analytics, and unrelated work. It expires when the scoped implementation is completed or abandoned and is historical evidence only, not a permanent expansion of Codex's repository role.
