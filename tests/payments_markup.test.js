...
describe('paid-surface markup (DOCTRINE §1 v0.22 / §6)', () => {
  ...
  it('paywall title and body carry the single-offer ownership framing (v0.56, permanence restored v0.63)', () => {
    ...
    expect(subtree).toMatch(/adds 11 coordinates, meanings on all but the moon cell, the written card, and domain fit/);
...
describe('disclosure copy (DOCTRINE §4 v0.22 / brief §10.3)', () => {
  ...
  it('about-modal: discloses what the offer buys, permanently (v0.55 ownership, restored v0.63)', () => {
    expect(aboutSubtree).toMatch(/opens the eleven sealed coordinates, a meanings panel on each but the moon cell, the written card entry \(name, type, habit, and one of three rotating note positions, first anchored by your life path\), and domain fit — permanently, for every reading in this browser/);
