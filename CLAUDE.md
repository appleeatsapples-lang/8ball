...
## Repository shape
    ...
    core/         pure functions — 14 modules (profile, engine, rising, moon, birthcard, pillars, countries, calendar, cities, payments, math, public, dyad, kua)
    ui/           ES modules — citysearch, concordance, dyad, labels, meanings, modals, pairShare, payments, profile, public, readings, result, share, sheet, tiers — DOM controllers use init*UI({refs},{hooks}) per §6 v0.23; concordance is pure post-calc lookup (the two-reading compare + the panels' intra-sheet filed-relation helper); public renders the t3-ceiling public read (§1.D v0.60 — t4 is retired); dyad renders the t5 paired read (§1.J) — "t3"/"t5" here name render-registry ceilings, not purchasable tiers: doctrine v0.71 (2026-09-02) retired the storefront, so every device renders every ceiling on every load and injects its own screen + CSS — since v0.76 also the paired compartments' panel, read through meanings' pure content path; v0.87 retires the label toggle in favor of permanently visible row labels; since v0.81 also the Pair Dossier hierarchy (heading/scope/signature/direction-explicit evidence/failure state/completion flow) and the screen-ownership exports (isOpen/compareAnother/currentRelation); sheet builds an instanced standalone specimen sheet, reusing tiers' cellRenderState so the dyad's two sheets and the host sheet cannot disagree; pairShare (§1.J/§5.D v0.81–v0.86) renders the Pair Imprint — a narrow, allow-listed share PNG for the paired reading, wired to dyad's currentRelation() getter PLUS a narrow onRelationChange notification (both hooks, never an import — the notification lets the PNG pre-render proactively for Web Share's transient-activation requirement); since v0.86 the share-view state is a model-first, non-recursive coordinator over one desired-view model for the shared button+status pair, not per-boundary DOM checkpoints (15 modules — kua's renderer was retired 2026-09-01 per the §1.D kua-retirement amendment; core/kua.js and content/kua.v1.js stay as registries) + two host stylesheets, not counted as modules: shell.css (the host shell styles, moved verbatim from index.html on 2026-08-31, linked FIRST) and experience.css (mobile/desktop experience layer, linked second)
    content/      dyad.v1.js (immutable t5 relation tables — 25 ordered element pairs, 6 branch registers, 9 ordered bracket registers; §1.J) + dyad.v2.js (ACTIVE — v1 tables unedited, master-preserving combined-path frames + provenance, §1.J v0.62) + cards.v1.full.js (144-card deck — HISTORICAL: JS-gated per §1 v0.22 behind a credits flag; current truth per doctrine v0.71's free amendment: every render shows the full card content, no gate) + meanings.v1.js (58 immutable historical entries, §1.G v0.44) + meanings.v2.js (superseded 1–9 view + element/context roles, §1.G v0.54) + meanings.v3.js (superseded-as-active twelve-value numerology registry — masters reused byte-for-byte from v1, §1.G v0.62; its tables remain the live tables via v4's re-export) + meanings.v4.js (superseded — v3 re-exported unedited plus the per-slot numerology lines and the theme-tension registry, §1.G; its tables remain live via v5's re-export) + meanings.v5.js (superseded — v4 re-exported unedited plus the rising/private-animal placement lines, §1.G; its tables remain live via v6's re-export) + meanings.v6.js (ACTIVE — v5 re-exported unedited plus the moon placement family and the moon context role, §1.K) + concordance.v1.js (immutable historical registry, §1.I v0.51) + concordance.v2.js (superseded nine-number registry, §1.I v0.54) + concordance.v3.js (ACTIVE twelve-value domain + the three master-reduction links, §1.I v0.62) + public.v1.js + public.v2.js (public-read tables — favorability, domain families, work modes, role postures; v2 carries v1 unedited and re-keys the mode to the birthday per §1.D v0.59; the read is a t3 ceiling block per §1.D v0.60) + public.v3.js (ACTIVE — v1/v2 tables unedited plus the declared master-to-base mode bridge, §1.D v0.62) + kua.v1.js (RETAINED eight-trigram registry — citation register + provenance incl. the named post-2000 limitation; its renderer was retired per the §1.D kua-retirement amendment, tables immutable per §4)
    agents/       agent role docs + platform constraints per §10 v0.24
    tests/        64 vitest files + fixtures.json + helpers/ (dom.js, voice-register.js — de-forked shared scan tables/mocks, non-test modules per §7)
    ...
    scripts/      render_cards.mjs — the in-repo renderer for the two artifacts that DEPICT the sheet: the spec_no-* / spec_extended_* specimen JPEGs (deterministic synthetic profiles derived from each code, the registry rows, §11) and assets/og-image.png (the share PNG's own card art); Chromium via a scratch-dir playwright-core (--driver), never a dependency · build_card_jpegs.py — deterministic PNG→JPEG renderer for the concordance + index families of cards/ (sources in the operator's vault) · extract_hko_fixture.py — regenerates the HKO fixture from the 200 upstream sources (two-step by design: re-extract, then update HKO_FIXTURE_CONTENT_SHA256 in project_audit.py in the same commit)
