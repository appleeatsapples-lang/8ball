...
## Repository shape
    ...
    core/         pure functions — 14 modules (profile, engine, rising, birthcard, pillars, countries, calendar, cities, payments, math, public, dyad, kua, moon)
    ui/           ES modules — citysearch, concordance, dyad, kua, labels, meanings, modals, payments, profile, public, readings, result, share, sheet, tiers — DOM controllers use init*UI({refs},{hooks}) per §6 v0.23; concordance is pure post-calc lookup; public renders the t3-ceiling public read (§1.D v0.60 — t4 is retired); kua renders the gender-keyed t3 kua block and injects its own style, block node and optional form control (§1.D kua amendment); dyad renders the t5 paired read (§1.J) and injects its own screen + CSS; sheet builds an instanced standalone specimen sheet, reusing tiers' cellRenderState so the dyad's two sheets and the host sheet cannot disagree (15 modules)
