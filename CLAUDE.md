...
## Repository shape
    ...
    core/         pure functions — 14 modules (profile, engine, rising, birthcard, pillars, countries, calendar, cities, payments, math, public, dyad, moon, measurement)
    ui/           ES modules — citysearch, concordance, dyad, labels, meanings, modals, payments, profile, public, readings, result, share, sheet, tiers — DOM controllers use init*UI({refs},{hooks}) per §6 v0.23; concordance is pure post-calc lookup; public renders the t3-ceiling public read (§1.D v0.60 — t4 is retired); dyad renders the t3 paired read (§1.J; §1.D v0.68 folded it into t3 and retired t5) and injects its own screen + CSS; sheet builds an instanced standalone specimen sheet, reusing tiers' cellRenderState so the dyad's two sheets and the host sheet cannot disagree (14 modules)
    content/      dyad.v1.js (immutable relation tables — 25 ordered element pairs, 6 branch registers, 9 ordered bracket registers; §1.J) + dyad.v2.js (ACTIVE — v1 tables unedited, master-preserving combined-path frames + provenance, §1.J v0.62) + cards.v1.full.js (144-card deck, JS-gated per §1 v0.22) + meanings.v1.js (58 immutable historical entries, §1.G v0.44) + meanings.v2.js (superseded 1–9 view + element/context roles, §1.G v0.54) + meanings.v3.js (ACTIVE twelve-value numerology registry — masters reused byte-for-byte from v1, §1.G v0.62) + concordance.v1.js (immutable historical registry, §1.I v0.51) + concordance.v2.js (superseded nine-number registry, §1.I v0.54) + concordance.v3.js (ACTIVE twelve-value domain + the three master-reduction links, §1.I v0.62) + public.v1.js + public.v2.js (public-read tables — favorability, domain families, work modes, role postures; v2 carries v1 unedited and re-keys the mode to the birthday per §1.D v0.59; the read is a t3 ceiling block per §1.D v0.60) + public.v3.js (ACTIVE — v1/v2 tables unedited plus the declared master-to-base mode bridge, §1.D v0.62)
    agents/       agent role docs + platform constraints per §10 v0.24
    tests/        57 vitest files + fixtures.json + helpers/ (dom.js, voice-register.js, js-lex.js — de-forked shared scan tables/mocks plus a JS lexical classifier for the static source guards, non-test modules per §7)
