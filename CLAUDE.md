...
## Repository shape
    ...
    ui/           ES modules — citysearch, concordance, dyad, labels, meanings, modals, payments, profile, public, readings, result, share, sheet, tiers — DOM controllers use init*UI({refs},{hooks}) per §6 v0.23; concordance is pure post-calc lookup (the two-reading compare + the panels' intra-sheet filed-relation helper); public renders the t3-ceiling public read (§1.D v0.60 — t4 is retired); dyad renders the t5 paired read (§1.J) and injects its own screen + CSS — since v0.76 also the paired sheets' labels toggle and their compartments' panel, read through meanings' pure content path; sheet builds an instanced standalone specimen sheet, reusing tiers' cellRenderState so the dyad's two sheets and the host sheet cannot disagree (14 modules — kua's renderer was retired 2026-09-01 per the §1.D kua-retirement amendment; core/kua.js and content/kua.v1.js stay as registries) + two host stylesheets, not counted as modules: shell.css (the host shell styles, moved verbatim from index.html on 2026-08-31, linked FIRST) and experience.css (mobile/desktop experience layer, linked second)
