...
2. Privacy scan — `tests/privacy_scan.test.js`. No unpermitted network calls (only DOCTRINE §5-permitted same-origin lazy loads and the §5.B user-initiated feedback POST; the dyad's commerce Buy Link redirect that doctrine v0.81 restored narrowly is retired again by PAYWALL-REMOVE-01, 2026-09-14 — the constant ships blank and no offer anchor ever gets an href; the signed `?dyad=` access link path is still wired but unreachable from any live UI, verified offline with Web Crypto, no call leaves the page); no third-party fonts or scripts; system fonts only. Saved Readings adds one doctrine-allow-listed local key; `tests/readings.test.js` locks its minimal schema and lifecycle. Concordance adds no key or schema field; `tests/concordance.test.js` locks its transient recomputation boundary and finite relation inventory.
