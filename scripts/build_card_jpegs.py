...
def is_specimen_code(code: str) -> bool:
    import re
    m = re.match(r"^spec_no-([mdclxvi]+)$", code)
    if m:
        return 1 <= _from_roman(m.group(1)) <= 144  # the 144-card grid, as parseCode()
    return bool(re.match(r"^spec_extended_[a-z-]+-\d{4}$", code))
...
def main() -> int:
    ...
    # ONE pass in queue order below (pr234 audit H2): splitting the queue
    # into two lists and appending them in turn regrouped the manifest and
    # broke tests/cards_hosting.test.js's exact-order pin.
    queue_codes = codes
    ...
    if check_only:
        ...
        try:
            ...
            # strict and ORDERED: the manifest is queue order end to end
    ...
    manifest_doc = {
        # the note has ONE owner, scripts/render_cards.mjs (MANIFEST_NOTE);
        # carried through here so the two writers never flip it (pr234 audit M1)
        "note": tracked_doc.get("note", "Generated by scripts/render_cards.mjs and scripts/build_card_jpegs.py."),
        "canvas": [CANVAS_W, CANVAS_H],
        "padding_rgb": tracked_doc.get("padding_rgb", [235, 229, 212]),
