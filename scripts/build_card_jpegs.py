...
SCOPE SINCE 2026-09-03: this script owns the concordance and index card
families only. The specimen sheets (spec_no-* / spec_extended_*) — the one
family that depicts the sheet — are rendered IN-REPO by
scripts/render_cards.mjs from the repo's own registries, so they can never
drift from the sheet again; this script skips those codes on write, leaves
their manifest entries untouched, and skips them on --check.
...
# Codes scripts/render_cards.mjs owns (the specimen sheets). Kept in sync with
# parseCode() there; the tracked manifest records their source as that script.
def is_specimen_code(code: str) -> bool:
    import re
    return bool(re.match(r"^spec_no-[mdclxvi]+$", code) or re.match(r"^spec_extended_[a-z-]+-\d{4}$", code))
...
def main() -> int:
    ...
    # The specimen sheets are scripts/render_cards.mjs's; carry their tracked
    # manifest entries through unchanged (they are not re-rendered here).
    tracked_doc = {}
    try:
        tracked_doc = json.loads((OUT_DIR / "manifest.json").read_text())
    except (OSError, ValueError):
        tracked_doc = {}
    tracked_by_code = {c["code"]: c for c in tracked_doc.get("cards", [])}
    specimen_codes = [c for c in codes if is_specimen_code(c)]
    codes = [c for c in codes if not is_specimen_code(c)]
    ...
    for code in specimen_codes:
        entry = tracked_by_code.get(code)
        if not entry:
            drift.append(f"{code}: specimen entry missing from manifest — run scripts/render_cards.mjs --specimens")
            continue
        manifest.append({"code": code, "source": entry["source"], "bytes": entry["bytes"]})
    for code in codes:
        src = sources[code]
    ...
    if check_only:
        ...
        try:
            ...
            # order-insensitive: the specimen entries are carried through
            # from the tracked manifest, the rest freshly rendered
            key = lambda c: c["code"]
            if sorted(tracked_cards, key=key) != sorted(manifest, key=key):
    ...
    manifest_doc = {
        "note": (
            "Generated by scripts/build_card_jpegs.py (concordance + index "
            "families, from the vault PNG sources) and scripts/render_cards.mjs "
            "(the spec_no-* / spec_extended_* specimen sheets, derived "
            "deterministically from each code and the repo registries — "
            "journal 2026-09-03). 'cards' cover the union of ALL FOUR reach "
            "surface queues (disjoint as of 2026-07-29). 'external' are queued "
            "codes hosted off-site by the vault's extra_specimens manifest and "
            "deliberately not rendered here; cards + external together cover "
            "every queued code. Every file is 1080x1350 JPEG, q90, metadata "
            "stripped."
        ),
