...
| 6 | Persistence | localStorage only — see DOCTRINE §5 for the canonical allow-list (current profile; labels/pending/tier state plus the read-only legacy-credits R2 signal (§1.D v0.55 — tries retired); t3 facet position `eight_ball_facet_index_v2` — v1 retired by calc v3, cleared once on first facet read; Saved Readings archive `eight_ball_saved_readings_v1`). Saved entries contain only local metadata plus the same user-entered reconstruction inputs as the current profile. Concordance selection/results are memory-only and add no key or schema field. No derived profile/card output stored. |
| 7 | Telemetry | No third-party / client telemetry. Permanently. (First-party server-side host logs are not telemetry — DOCTRINE §5.) |
| 8 | Calc version | v3 — strict Pythagorean numerology 1–9; tropical sun; real lunar new year + solar-term tables (1900–2100, Asia/Shanghai date-precision) |
...
**Current meaning amendment (supersedes row 9's v0.44 phrase and the v0.52 provenance-only treatment):** §1.G v0.54 makes all fourteen sheet compartments interactive and gives every resolved value a registered meaning plus deterministic context. Numerology exposes exactly numbers 1–9 and reads each of its six axes alongside other numerology axes under `with the other numbers`. Unresolved/sealed states and catalog exclusion remain explicit.
...
- **Life path:** Pythagorean. Sum digits of YYYY-MM-DD and repeatedly reduce to 1–9.
- **Name number:** Pythagorean letter values (a=1..i=9, j=1..r=9, s=1..z=8), repeatedly reduced to 1–9.
- **Soul urge:** Pythagorean vowel sum, repeatedly reduced to 1–9.
- **Personality:** Pythagorean consonant sum, repeatedly reduced to 1–9.
- **Birthday:** Day-of-month, repeatedly reduced to 1–9.
- **Maturity:** Reduced life path plus reduced name number, then repeatedly reduced to 1–9.

If a name has no contributing letters for a subset (no standard vowel for soul urge or no consonant for personality), that coordinate is unresolved (`—`) rather than displayed as `0`; the active system never invents a tenth number.
