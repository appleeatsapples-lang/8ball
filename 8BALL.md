...
| `ui/`             | 13 modules: `payments.js` (paywall/paid-return controller + t3 current-facet storage), `profile.js` (current-profile persistence + form helpers), `readings.js` (Saved Readings v1 storage + Previous Readings / selection / comparison screens), `concordance.js` (pure post-calculation registry lookup), `share.js` (on-device PNG/share), `tiers.js` (compartment render + share/provenance/atlas/density — since v0.74 the provenance/atlas REGISTRIES; their card writers retired), `labels.js` (symbol-label toggle), `meanings.js` (all-cell value meaning + deterministic sheet context), `public.js` (public-reading formatter + bridge disclosure surface), `dyad.js` (second-profile entry and paired-sheet render), `sheet.js` (shared sheet value mapping/render helpers), `modals.js` (about/forget + shared focus helpers), `citysearch.js` (city autocomplete). DOM controllers use the `init*UI({refs}, {hooks})` DI shape per DOCTRINE §6 v0.23; pure relation logic has no DOM/storage/network capability. |
