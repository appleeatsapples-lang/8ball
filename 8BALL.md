...
| `ui/`             | 10 modules: `payments.js` (paywall/paid-return controller + t3 current-facet storage), `profile.js` (current-profile persistence + form helpers), `readings.js` (Saved Readings v1 storage + Previous Readings / selection / comparison screens), `concordance.js` (pure post-calculation registry lookup), `share.js` (on-device PNG/share), `tiers.js` (compartment render + share/provenance/atlas/density), `labels.js` (symbol-label toggle), `meanings.js` (tradition-cited meaning panel), `modals.js` (about/forget + shared focus helpers), `citysearch.js` (city autocomplete). DOM controllers use the `init*UI({refs}, {hooks})` DI shape per DOCTRINE §6 v0.23; pure relation logic has no DOM/storage/network capability. |
