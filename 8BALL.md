...
**v0.3.0 IN-FLIGHT on branch `v0.3.0-depth-unlock`.** First paid surface in the product — three free reads, then $3 buys three reads with the card opened up (name, type, habit, bracket-resolved note from `~/dev/8ball-private/cards.v1.full.js`). Hard paywall at try 4. Lemon Squeezy hosted checkout via plain `<a href>` Buy Link (no JS network surface; LS handles payment + email, on-device stores name/DOB/reading). Steps 1–9 of 12 shipped on the branch; steps 10–12 pending. Architecture additions: `core/payments.js` (pure state machine — isNewPair, nextShakeState, applyPaidReturn), `ui/payments.js` (165 LOC paid-surface controller — LS keys, paywall modal handlers, `?paid=t1` redirect handler), `ui/profile.js` (153 LOC — profile persistence + form helpers, extracted from index.html at step 7 to absorb codex pre-merge audit hook 8 P1 on the 1500-line ceiling). New localStorage keys (§5 allow-list extensions): `eight_ball_tries_used_v1`, `eight_ball_credits_v1`, `eight_ball_pending_profile_v1`. Doctrine bumped v0.21 → v0.22 at step 1 (paid-surface clauses §1.B / §4.B / §5.B / §6 / §7); final v0.23 bump bound for step 11 (locks `ui/*.js` split-target wording with both modules as established precedent). Pre-locked decisions still in force: Pattern C visual · 3 free tries · 3 reads per $3 · Tesla-369 staircase pricing · public bundle delivery (deck visible in source, lock is convention) · LS redirect not overlay · β try-counting (re-shake same profile is idempotent) · pending-profile mechanic (Path A + B) · `try another` = `resetFormDisplay` not `clearProfile`. About-modal copy expanded to ~180 words at step 7 with Lemon Squeezy disclosure + on-device data-flow boundary + source-visibility framing. Step 8 bundled two CiC verifier fixes: numerology triplet always space-separated (was hasMaster-branched) + future-DOB validation (HTML5 `max=` soft fence + JS submit-handler real gate + inline `.field-error` markup). Step 9 closed the deferred 4 JS-pattern groups in `tests/payments_markup.test.js`. Tests 502 → 583 (+81 across the cycle). index.html 1498 (pre-cycle) → 1441 (post-step-8) — sub-ceiling by 59 lines. Local PII audit clean. Aesthetic mono per Phase-2E. Lane discipline: chat-Claude orchestrator + direct-write via Desktop Commander; Codex adversarial audit between step 6 and step 7 (mid-cycle two-eyes preservation per §10) cleared 8 PASS / 2 P1 / 0 P0, both P1s absorbed at step 7. Bound for steps 10–12: 8BALL.md + journal updates (this step), doctrine v0.23 final bump, PR open + Codex full-PR audit + live-fire on Netlify deploy preview + LS Test→Live swap + merge.
