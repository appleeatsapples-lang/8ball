...
ONE static site. No backend. No accounts. No telemetry. Saved Readings is browser-local only.
...
| `ui/`             | 9 modules: `payments.js` (paywall/paid-return controller + t3 current-facet storage), `profile.js` (current-profile persistence + form helpers), `readings.js` (Saved Readings v1 storage + Previous Readings screen/dialog), `share.js` (on-device PNG/share), `tiers.js` (compartment render + share/provenance/atlas/density), `labels.js` (symbol-label toggle), `meanings.js` (tradition-cited meaning panel), `modals.js` (about/forget + shared focus helpers), `citysearch.js` (city autocomplete). DOM-touching ES modules with `init*UI({refs}, {hooks})` DI shape per DOCTRINE §6 v0.23; several also expose pure testable helpers. |
