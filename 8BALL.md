...
A magic 8-ball that knows you. **Current-surface spec: read `README.md` (top paragraph) + `DOCTRINE.md §1.D` (tier ladder, as amended through v0.68) + `DOCTRINE.md §1.G` (coordinate meaning, as amended through v0.53) + `DOCTRINE.md §1.H` (t3 written-entry rotation, v0.49) + `DOCTRINE.md §1.I` (Registry + Concordance, v0.51) FIRST — this paragraph is orientation, not the spec, and defers to those on any disagreement.** Enter name + DOB once, optionally add birth time + city (autocomplete from `assets/cities.json`, sets IANA tz + lat + lng atomically — see DOCTRINE §1.A v0.21), shake. The free card surfaces five calibrated coordinates from the date of birth alone — tarot birth card, sun sign, public animal (year-pillar), life path, catalog numeral — rendered on a constant compartment specimen sheet (§1.D v0.37): every coordinate has a cell, and cells above the device's tier show as sealed hatches (withheld, not absent). Fourteen of the fifteen sheet compartments are pointer- and keyboard-interactive (the moon compartment ships without a meanings panel — a disclosed §1.K residual) (§1.G v0.53). Each resolved value opens its registered meaning plus a separately labeled deterministic reading of how it works beside two resolved neighboring coordinates; unresolved and sealed cells do not fabricate or leak a meaning. The catalog numeral remains excluded. Three paid rungs ($1 / $2 / $3 — §1.D v0.36 ladder, v0.55 ownership repricing; one-time, permanent, unlimited) open the rest of the sheet; the name enters the math at the first rung: t1 = rising sign and moon sign (both when computable, §1.K) · five-element · private animal (month-pillar) · the name-derived numerology pair (name number + soul urge); t2 = + personality · birthday · maturity · day pillar; t3 = + hour pillar · the written 144-card entry · domain fit (the public read, folded in at §1.D v0.60) · the comparative — a second person's complete sheet plus the named relation between the two (folded in at §1.D v0.68, so one $3 price carries it). An explicit t3 `flip again` rotates the entry through its three immutable v1 note positions (owned, unlimited — §1.H v0.55); lower-tier flips remain cosmetic. Underneath, the (sun sign, public animal) pair drives a 144-card catalog index — computed positionally by `core/engine.js` (sun-row × 12 + animal-col + 1, roman numeral). All other coordinates are surface-only and never enter the catalog driver; life path anchors the first visible note position (low/mid/high) within a card cell, not the index. The card content (name, type, habit, note per cell × low/mid/high positions) ships in the public bundle at `content/cards.v1.full.js`, JS-gated by the stored tier per DOCTRINE §1.D / §5.C v0.55 — the written entry opens and rotates only at t3; private authoring source is preserved at `~/dev/8ball-private/cards.v1.full.js`.
...
| 9 | Content version | v0.3.0-public-tier-1 (catalog + JS-gated card-content layer at `content/cards.v1.full.js`; compartment specimen-sheet render at every tier — sealed cells carry row/label structure, never values (§1.D v0.37); free surface = five DOB-derived coordinate values (§1.D v0.38) — tarot birth card (Major Arcana numeral + name, digit-sum reduction, §1.C, v0.5.0) · sun · public animal (year) · life path · catalog numeral; four of those five are tappable to a static tradition-cited meaning entry at `content/meanings.v1.js` (§1.G v0.44 — catalog excluded, that compound reading is the t3 paid entry); three-rung entitlement ladder (§1.D as amended through v0.68): t1 = rising (when computable) · moon (same birth-time+place gate, §1.K) · five-element · private animal (month) · name-derived numerology pair (name number + soul urge, §1.D v0.38 / §1.B v0.43 — the pre-v0.38 text triplet is superseded lineage); t2 = + personality · birthday · maturity number trio · day pillar; t3 = + hour pillar · the written 144-card entry (name/type/habit/note × low/mid/high) · the public read (domain fit) · the §1.J comparative (a second complete sheet + the relation layer, folded in at §1.D v0.68); t5 = RETIRED into t3 (§1.D v0.68) — the comparative it named is now part of the $3 rung above; a stored `t5` migrates to `t3`, which is a superset, so nobody is downgraded; explicit t3 `flip again` rotates the immutable v1 note positions, unfunded and unlimited (§1.H v0.49 as amended v0.55), with current position stored in `eight_ball_facet_index_v3` (v0.62 calc-v4 key versioning; `_v1` and `_v2` are retired generations, cleared once on the first v3-key read); symbol-label visibility toggle; provenance placards (per-coordinate derivation note, labels-gated, surface-only, §1.E v0.40); atlas system-name legend (per-coordinate, labels-gated, §1.F v0.41); density census (always-on result-rail aggregate, off-PNG, §1.F v0.41); opt-in feedback surface §5.B Call 1; Gumroad Buy Link redirect §5.B Call 2 (LS path retired v0.3.0.3); open free surface — unlimited readings, cap retired (§4.B v0.55); content-delivery transparency §5.C; share surface §5.D — full specimen sheet (open coordinate values + sealed-compartment hatch structure) + catalog + wordmark → on-device PNG, Web Share API with a clinical caption, v0.39; private authoring source preserved at `~/dev/8ball-private/cards.v1.full.js`; 18+ acknowledgment gate §4.A retired v0.48, 2026-07-06) |
