...
| `tests/`          | 24 test files + `fixtures.json` as of 2026-06-30 (1115 cases): calc + engine pipeline (`profile`, `rising`, `cities`, `countries`, `numerology_display`, `labels_reveal`, `age_gate`, `dob_validation`, `payments_markup`, `tiers`); privacy (`privacy_scan`); PII (`pii_scan`); dependency discipline (`dependency_discipline`); payments state machine (`payments_state`); feedback surface (`feedback_surface`); share surface (`share_surface`); tarot birth-card (`birthcard`); rising-input disclosure (`rising_disclosure`); four-pillars day+hour (`pillars`); prose coordinate-count (`prose_coordinate_count`); CLP legibility surfaces (`provenance` §1.E, `atlas` + `density` §1.F); §6 modal split (`modals`). See DOCTRINE §7 for the 6-stage CI breakdown. |
