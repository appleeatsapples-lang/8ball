...
| 9 | Content version | v0.2.6-public (catalog-only; optional rising-sign surface coordinate; numerology text triplet surface; positional math in engine.js; full content private at `~/dev/8ball-private/cards.v1.full.js`; launch-prep meta + favicons + og:image at /assets/; opt-in feedback surface §5.B; 18+ acknowledgment gate §4.A) |
...
**v0.2.7 — labels-reveal + prestige refinements (deferred from v0.2.6).** CC lane. File-touch list and decisions locked at session start (operator pick `1A 2A 3A 4B`); see `~/Desktop/8ball/sessions/handoff_v026_label_reveal_prestige.md`. Pre-flight catches to land in the v0.2.7 brief: title typography is 9px/0.22em (literal form-field-label match) not 11px/0.1em as prior handoff claimed; storage key is `eight_ball_labels_revealed_v1` to match `eight_ball_profile_v1` namespace.
