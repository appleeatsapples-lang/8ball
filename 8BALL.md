...
**Metaphorically: SIRR is the engine, 8-Ball is the deniability layer. Architecturally: siblings — no code shared, no imports.** SIRR (sacred, private, personal) and 8ball (materialistic, public, commercial) are sibling repos. They share NOTHING in code. They share calculation rigor by example, not by import. (See `DOCTRINE.md §9`.)
...
| Folder | Contents |
|---|---|
| `core/`           | `profile.js` (calc — sun, animals, life path, name number, soul urge, personality, birthday, maturity), `engine.js` (positional catalog + bracket resolution), `rising.js` (ascendant math), `countries.js` (legacy v0.2.1 fixed-offset country entries — backward-compat for stored profiles), `calendar.js` (Meeus lunar new year + solar terms 1900–2100), `cities.js` (city autocomplete loader for `assets/cities.json`), `payments.js` (pure state machine — `isNewPair`, `nextShakeState`, `applyPaidReturn`). Pure functions, no DOM. |
| `ui/`             | `payments.js` (paywall modal controller + `?paid=t1` handler — added v0.3.0), `profile.js` (profile persistence + form helpers — extracted from `index.html` at v0.3.0 step 7 to absorb codex pre-merge audit hook 8 P1). DOM-touching ES modules with `init*UI({refs}, {hooks})` DI shape per DOCTRINE §6 v0.23. |
...
| `tests/`          | 14 test files + `fixtures.json` as of v0.3.0: calc + engine pipeline (`profile`, `rising`, `cities`, `countries`, `numerology_display`, `labels_reveal`, `age_gate`, `dob_validation`, `payments_markup`); privacy (`privacy_scan`); PII (`pii_scan`); dependency discipline (`dependency_discipline`); payments state machine (`payments_state`); feedback surface (`feedback_surface`). 586 cases as of v0.3.0+. See DOCTRINE §7 for the 6-stage CI breakdown. |
...
## 10. Current state (as of 2026-05-13)
...
11. **v0.3.1 — SHAKE AGAIN facet re-roll (paid-surface depth re-roll).** Design-locked chat-12 (2026-05-13). Paper-design pass + cycle brief on disk: `~/Desktop/8ball/sessions/v0.3.x_shake_again_facet_reroll.md` (parking doc, 273 lines, chat-6 + chat-12 layers) + `~/Desktop/8ball/sessions/brief_v031_facet_reroll.md` (cycle brief, 351 lines, three-lane scope: ChatGPT authoring + CC implementer + Codex audit with extractable PROMPT START/END blocks per lane). Sub-path c.2 locked (re-author existing 3 brackets per cell as lateral lenses); 4 of 5 listed sub-decisions resolved with recommendations (selection rule = round-robin no-immediate-repeat; first-shake anchor = LP-bracket-derived; storage = new `eight_ball_facet_index_v1` key); **one open operator-taste decision — sub-decision #6 lateral facet taxonomy** (recommended lock: outward / inward / returning per chat-6 sample probe; alternatives: action/feeling/pattern; gift/cost/return; persona/shadow/anima). Doctrine amendment text tightened chat-12 with routing correction (chat-6 parking doc's "§6.5/§7.1 amendment" framing referenced non-existent subclauses — β-idempotence currently lives only as a test invariant in §7 stage 6, never doctrine-codified): NEW §1.C (paid-surface facet rotation, mirrors §1.A rising / §1.B numerology surface-clause pattern) + surgical edits at §4 v0.22 amendment text / §5 allow-list / §7 stages 1+3+6. Bump v0.27 → v0.28 proposed. Schema migration `cards.v1.full.js` → `cards.v2.full.js` per §4 immutability. **Ship-gate: signal-gated, not time-gated** — requires (a) operator taxonomy lock on sub-decision #6, (b) LS Live unlock from identity verification (banner-state per LS dashboard 2026-05-13), (c) ≥5 real paid purchases on Live LS with observed SHAKE AGAIN behavior. Pre-traction design lock; post-traction ship trigger. Sibling cosmetic ticket parked at `~/Desktop/8ball/sessions/v0.3.x_paint_gap_paid_return.md` — piggybacks onto v0.3.1 surface touch, not standalone.
