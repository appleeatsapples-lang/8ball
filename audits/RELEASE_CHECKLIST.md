...
- [ ] **Live smoke test** (verifier lane via CiC, or operator-eyes). Run the paths the release touched:
  ...
  - Paid-return persistence — the stored rung (`eight_ball_tier_v1`) survives reload and re-shake at the entitled density; monotonic (a higher rung bought later upgrades, never downgrades — highest rung bought holds). t3 additionally opens the hour pillar + the written card entry (name/type/habit/note). At t3, each funded explicit `flip again` advances exactly one v1 note position and debits exactly one credit; reload preserves the visible position. Free/t1/t2 flips remain cosmetic with zero debit. A zero-credit t3 flip clears stale pending intent and opens the paywall without changing the position; the return grants credits without auto-consuming a facet, and the next explicit flip performs the advance/debit.
