...
- [ ] **Live smoke test** (verifier lane via CiC, or operator-eyes). Run the paths the release touched:
  ...
  - Pair Imprint (v0.79, §1.J/§5.D; second remediation gate 2026-09-04) — the pre-action disclosure text is truthfully CONDITIONAL, never an unconditional promise: `shares directly when your device supports it, otherwise saves as an image` when the platform exposes native share methods, plain `saves the pair imprint as an image on this device` when it doesn't (method presence alone never implies a share will succeed). The PNG is pre-rendered and cached the instant a resolved relation becomes current (a narrow `onRelationChange` hook from ui/dyad.js, invalidated on close/Compare Another/Previous Readings/any replacement) so `navigator.share()` can be called SYNCHRONOUSLY inside the click handler with no prior `await` — required for the Web Share API's transient-activation rule; native share is attempted ONLY when that cached blob is already ready at click time, never after a wait. The share button goes `aria-busy="true"` and disables both while the artifact is being prepared (background pre-render) and while an active click is in flight; a changed or closed pair during ANY await (a pending render, `navigator.share()` resolving OR rejecting, `clipboard.writeText()` resolving OR rejecting) reports `stale` and lands no side effect for the old pair — checked after every one of those async boundaries, not just the first. A missing/throwing `File` constructor, or `navigator` absent entirely, always falls through to the on-device PNG download — never a bare "failed". A non-AbortError share rejection preserves the local download rather than giving up. Each `initPairShareUI()` call is isolated in its own controller object; a re-init retires the prior one so its in-flight completion can never write into new refs or admit a third click. The constructed snapshot never carries a name, DOB, birth time, timezone, latitude, longitude, city, either raw sheet, or written card text — only the three allow-listed relation summaries plus fixed copy — and the three signature values are geometry-fitted so the longest real line (enumerated from the actual content tables) never overflows its text lane.
