...
User taps `unlock · $3` on the paywall modal → browser navigates to `https://8-ball.lemonsqueezy.com/checkout/buy/<variant-id>?checkout[success_url]=https%3A%2F%2Fthe-eight-ball.netlify.app%2F%3Fpaid%3Dt1` (hosted by Lemon Squeezy, which acts as Merchant of Record). The URL-encoded `checkout[success_url]` query parameter on the Buy Link href routes post-payment redirect back through the §5.B Call 2 contract; the LS dashboard's per-product Confirmation Button link is also set to `https://the-eight-ball.netlify.app/?paid=t1` as belt-and-suspenders (v0.3.0.2 ship discipline — code-tracked carries through Test+Live without dashboard-config drift; dashboard Button link covers the case where the query-param mechanism is undocumented per LS). LS processes payment (Stripe under the hood for cards; PayPal at buyer option). On successful payment, LS redirects to `https://the-eight-ball.netlify.app/?paid=t1`; on return, the page-load JS handler parses the query, calls `applyPaidReturn()` from `core/payments.js` to credit the device (`+= 3`) and — if `eight_ball_pending_profile_v1` is present — consume it (decrement one credit, save the profile, render unlocked), then `replaceState({}, '', window.location.pathname)` strips the query for a clean URL.
