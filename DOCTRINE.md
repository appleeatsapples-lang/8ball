...
The result surface MAY offer a user-initiated share affordance that renders the free, symbols-only card to a raster image (PNG) entirely on-device (SVG → canvas → `toBlob`; no network). Sharing uses the Web Share API where available (`navigator.share` with the image file); otherwise it falls back to a local file download plus a clipboard copy of the bare production URL. Invariants:

- **(a)** the shared artifact contains only the free coordinate symbols and the bare production URL — never the paid card-content layer (name/type/habit/note), never name or DOB;
