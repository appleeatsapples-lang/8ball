...
**doctrine version:** 2026-05-30 · v0.32 (§13 cadence amended + §7 mirror — rule-kill review weekly → monthly. Weekly review at solo cadence produced zero pruning (all reviews 2026-05-08 → 2026-05-30 were all-KEEP calibration; rules younger than the 30-day archive threshold have no firing surface) and silently lapsed two consecutive Fridays (2026-05-22 + 2026-05-29) — the prune-discipline became the rule that stopped firing. Monthly aligns the review interval to the 30-day threshold it enforces and co-locates with other monthly beats (next: 2026-06-15, with the v0.3.1 gate-close read). Sibling change: `agents/orchestrator.md` gains Procedure 9 (re-grep canonical state before orienting/drafting) per the orchestrator-stale-context L promoted same session (2 sightings: chat-25 §5.C duplicate-analysis / 2026-05-30 handoff-orientation-shortcut). The Friday pre-stage's §12 "trait pool / template expansion" AMEND leg was DROPPED on verification — that string no longer exists in DOCTRINE (retired earlier); the surviving §12 "trait pool" multi-language clause is accurate, not drift. Doctrine-governance-only cycle: no code, no calc, no `content/` deck touch, no shipped-surface change, no new dependency, no test logic change — `tests/pii_scan.test.js` DOCTRINE_ALLOW unchanged, `tests/privacy_scan.test.js` unchanged.)
